            ...
            const commitFilesByType = (type) => __awaiter(this, void 0, void 0, function* () {
                ...
                if (files.length) {
                    ...
                    const breakingMessage = breaking
                        ...
                        : "";
                    yield commit(`${commitTypes[type]}(icons)${breaking ? "!" : ""}: ${type} ${files.join(", ")}${breakingMessage} [skip ci]`, files.map((item) => path.join(SVG_FOLDER, item)));
                }
            });
...
const getFigmaSvgUrls = (fileKey, nodeId) => __awaiter(void 0, void 0, void 0, function* () {
    const nodes = yield figmaAPI_1.getNode(fileKey, nodeId);
    const nodeIds = encodeURIComponent(nodes.map((item) => item.id).join(","));
    const imageUrlsWithId = yield figmaAPI_1.getSvgs(fileKey, nodeIds);
    const results = [];
...
const downloadAllSvgs = (svgUrls) => {
    const downloadAll = svgUrls.map((item) => utils_1.download({
        name: `${item.name}.svg`,
        url: item.url,
        path: TEMP_SVG_PATH,
    }));
