      ...
      `);
            ...
            if (added.length || modified.length) {
                ...
                if (added.length) {
                    ...
                    yield commit(`new(icons): add ${added.join(", ")}`, added.map((item) => path.resolve(SVG_PATH, (item.name1 || item.name2))));
                }
                if (modified.length) {
                    core.info(`update ${modified.join(", ")}`);
                    yield commit(`update(icons): update ${modified.join(", ")}`, modified.map((item) => path.resolve(SVG_PATH, (item.name1 || item.name2))));
                }
            }
            yield push();
        }
