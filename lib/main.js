...
BREAKING CHANGE: ${deleted.join(", ")} was deleted!`, deleted.map((item) => path.join(SVG_FOLDER, item)));
            }
            if (added.length || modified.length) {
                ...
                if (added.length) {
                    ...
                    yield commit(`new(icons): add ${added.join(", ")}`, added.map((item) => path.join(SVG_FOLDER, item)));
                }
                if (modified.length) {
                    core.info(`update ${modified.join(", ")}`);
                    yield commit(`update(icons): update ${modified.join(", ")}`, modified.map((item) => path.resolve(SVG_FOLDER, item)));
                }
            }
            yield push();
        }
