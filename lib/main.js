...
const SVG_PATH = path.resolve(process.env.GITHUB_WORKSPACE, core.getInput("svg_folder_path") || "svg");
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (deleted.length) {
                ...
                utils_1.deleteFiles(deleted, SVG_PATH);
...
BREAKING CHANGE: ${deleted.join(", ")} was deleted!
      `);
            ...
            if (added.length || modified.length) {
                yield io.mv(TEMP_SVG_PATH, SVG_PATH);
                yield io.rmRF(TEMP_SVG_PATH);
                if (added.length) {
                    core.info(`add ${added.join(", ")}`);
                    yield commit(`new(icons): add ${added.join(", ")}`, added.map((item) => path.resolve(SVG_PATH, item)));
                }
                if (modified.length) {
                    core.info(`update ${modified.join(", ")}`);
                    yield commit(`update(icons): update ${modified.join(", ")}`, modified.map((item) => path.resolve(SVG_PATH, item)));
                }
            }
            yield push();
        }
